...
在 [#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17) 贡献的银行系统基础上，合并 main 前的一轮优化：

- 强制收款改为 钱包 → 银行活期 → 强制解约定期 依次穿透扣款，可通过 `loan.collect_from_fixed` 关闭
- 欠税形成闭环：滞纳金、先还旧账后缴新税、`/钓鱼还税` 主动缴清、欠税期间禁止转出资产、后台可减免
- `tax.asset_scope` / `deduct_scope` 默认值恢复为 `wallet`，与 #17 的原始设定一致（银行作为资产税缓冲）
- 大额取款预约新增过期时间，超时自动作废并释放被锁定的资金
- 关闭银行改为只封锁入金，取款与定期领取保持可用，玩家资产不会被锁死
- 新增 `bank_transactions` 流水表，存取、利息、手续费、违约金、欠税全部留痕，后台可查
- 银行仓储改用统一连接管理器（单事务 + 锁冲突重放），时间基准统一为 UTC+8
- 后台税收配置写回改走框架接口并精确定位自身配置文件，不再误伤同机其它插件实例；表单加 CSRF 校验
- 新增 `/钓鱼还税`、`/钓鱼银行 流水`、`/排行榜 总资产`
...
| `/钓鱼还税 [金额]` | `/还税`、`/钓鱼银行 还税` | 直接从钱包缴清欠税（不填金额则全额缴清） |
| `/钓鱼银行 流水` | `/钓鱼银行 账单`、`/钓鱼银行 明细` | 查看最近的银行资金流水 |
| `/排行榜 总资产` | `/排行榜 资产`、`/排行榜 财富` | 按 钱包+银行活期+定期本金 排名 |
...
- 银行的定位是**资产税缓冲 + 金币蓄水池**：默认情况下银行活期与定期本金都不计入每日资产税（`tax.asset_scope` 默认 `wallet`），玩家把金币存入银行可以规避资产税。这是既定的玩法设计；不希望如此的服务器把 `asset_scope` 改成 `wallet_bank` 或 `wallet_bank_fixed` 即可
- 存款免费并即时到账
- 每日免费提现额度默认 1,000,000 金币
- 超出免费提现额度的部分默认收取 3% 取款手续费
- 取款手续费按扣款当时的当日剩余免费额度计算；预约取款展示的是预估费用，确认时会重新计算
- **单笔**取款达到 5,000,000 金币需要先预约，默认 24 小时后可确认取出。门槛按单笔判定，连续多笔小额取款不受限制；需要收紧的服务器可直接调低 `reservation_threshold`
- 预约期间对应资金会被锁定：既不能取款也不能开定期，催收同样不会动它
- 预约在可确认时间之后 72 小时（`reservation_expire_hours`）仍未确认会自动作废并释放锁定资金
- 支持 1/3/7/30 天定期存款，默认收益率为 0.1%/0.4%/1.0%/5.0%
- 定期本金从银行活期余额转入，锁定期间不能取款；到期未领取的存单会在每日结算时自动入账活期（`auto_settle_matured`）
- 定期提前取出不获得收益；本金超过 1,000,000 金币的大额存单默认扣除 1% 违约金
- 关闭银行（`bank.enabled = false`）**只封锁入金**，取款、定期领取和提前取出仍然可用，避免玩家资产被锁死
- 所有资金变动都会写入 `bank_transactions` 流水表，包括被销毁的手续费/违约金与凭空产生的定期利息，可在后台「银行管理」页查看

**🧾 每日资产税规则**

- 税收配置位于插件配置的 `tax` 节，可在 Web 后台“税收管理”页面直接调整
- `asset_scope` 控制每日资产税统计范围：`wallet` 仅钱包（**默认**，银行活期与定期本金免税），`wallet_bank` 钱包+银行活期，`wallet_bank_fixed` 钱包+银行活期+进行中定期本金
  - 默认值下玩家可以把金币存入银行来规避资产税，这是银行作为「资产税缓冲」的设计意图
  - 想防止「0 点前存银行避税」，改成 `wallet_bank`；连定期本金一并计税则用 `wallet_bank_fixed`
- `deduct_scope` 控制每日资产税扣款来源：`wallet` 仅钱包（**默认**），`bank` 仅银行活期可用余额，`wallet_bank` 钱包优先、不足再扣银行活期可用余额
...
- 上面三个口径都有白名单校验：填了非法值会在日志里告警并回落默认，不会静默换一套算法
- 可扣来源不足时会产生欠税。**欠税的完整闭环**：
  - 每天结算时先按 `debt_surcharge_rate`（默认 2%/天）计滞纳金，同一结算日只计一次
  - 然后**先还旧账后缴新税**：可扣资产优先冲抵历史欠税，剩下的才用来缴当日税，仍不足的转为新欠税
  - 银行取款、预约确认、定期领取/提前取出时同样会优先补扣
  - 玩家可用 `/钓鱼还税` 直接从钱包缴清
  - 欠税未清时禁止存款、开定期和转账（`bank.block_inflow_when_in_debt`，默认开），否则把钱转给小号就能永远躲开补扣
  - 管理员可在后台「税收管理」页减免或清零欠税
