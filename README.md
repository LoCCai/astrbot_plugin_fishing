...
🧾 **资产税可配** - 钱包/活期/定期本金默认全部计税，计税范围与扣款来源均可调整<br>
🔒 **避税通道收口** - 定期计税、预约超时自动解锁、催收穿透银行、取款门槛按当日累计<br>
...
- 每日资产税默认统计范围改为 `wallet_bank_fixed`，进行中的定期本金一并计税，堵死「定期既免税又生息」
- 强制收款改为 钱包 → 银行活期 → 强制解约定期 依次穿透扣款，借款人存进银行不再能躲过催收
- 大额取款预约门槛改为按当日累计取款额判定，不再能靠拆单绕开
...
- 钱包、银行活期、进行中的定期本金**全部计入**每日资产税（`tax.asset_scope` 默认 `wallet_bank_fixed`）
- 存款免费并即时到账
- 每日免费提现额度默认 1,000,000 金币
- 超出免费提现额度的部分默认收取 3% 取款手续费
- 取款手续费按扣款当时的当日剩余免费额度计算；预约取款展示的是预估费用，确认时会重新计算
- **当日累计**取款达到 5,000,000 金币需要先预约，默认 24 小时后可确认取出。按累计而非单笔判定，否则连续取「门槛-1」就能完全绕开预约
...
- `asset_scope` 控制每日资产税统计范围：`wallet` 仅钱包，`wallet_bank` 钱包+银行活期，`wallet_bank_fixed` 钱包+银行活期+进行中定期本金（**默认**）。选 `wallet` 等于允许玩家把金币存入银行完全规避资产税；选 `wallet_bank` 则定期仍是免税通道——1 天期定期既能避税又能生息，玩家的最优解会变成把全部资产滚定期
- `deduct_scope` 控制每日资产税扣款来源：`wallet` 仅钱包，`bank` 仅银行活期可用余额，`wallet_bank` 钱包优先、不足再扣银行活期可用余额（默认）
