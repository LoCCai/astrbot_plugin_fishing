...
- 银行活期默认参与每日资产税（`tax.asset_scope` 默认 `wallet_bank`）；如需让银行余额免税，需显式改为 `wallet`
- 进行中的定期本金默认不计税，改为 `wallet_bank_fixed` 后一并计入
- 存款免费并即时到账
...
- `asset_scope` 控制每日资产税统计范围：`wallet` 仅钱包，`wallet_bank` 钱包+银行活期（默认），`wallet_bank_fixed` 钱包+银行活期+进行中定期本金。选 `wallet` 等于允许玩家把金币存入银行规避资产税
