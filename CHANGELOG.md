...
合并 dev 前对 v2.6.4 新增功能的一轮优化，重点是堵住银行带来的避税/赖账通道，
并把「钱去哪了」这件事变得可审计。

本版优化的对象是 [@LoCCai](https://github.com/LoCCai) 在
[#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17) 中贡献的银行系统、
资产税范围配置与 SQLite 事务加固，银行、税收、借贷的主体实现均出自该 PR，再次感谢。

- **🔒 堵死三条避税/赖账通道**：
  - 每日资产税的默认统计范围由 `wallet_bank` 改为 `wallet_bank_fixed`，进行中的定期本金一并计税。此前定期既免税又生息，玩家的最优解是把全部资产滚 1 天定期，税收系统实际失效
  - 强制收款改为按 钱包 → 银行活期可用余额 → 强制解约定期（`loan.collect_from_fixed`，默认开）的顺序穿透扣款。此前催收只看钱包，借款人借完立刻存进银行即可让催收颗粒无收
  - 大额取款预约门槛改为按**当日累计**取款额判定。此前只看单笔，连续取「门槛-1」就能完全绕开 24 小时预约

- **🧾 欠税从此有始有终**：
  - 每日税收改为「先还旧账后缴新税」：先计滞纳金，再用可扣资产冲抵历史欠税，剩余部分缴当日税。此前只挂账不清偿，欠税会永远滚下去
  - 新增 `tax.debt_surcharge_rate`（默认 2%/天）欠税滞纳金，同一结算日只累计一次
