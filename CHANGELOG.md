...
## v2.6.7 (容量升级独立化 + Web 配置归位与实时生效)

- **🐠 鱼塘与水族箱升级配置独立化**：
  - 水族箱默认档位迁入 `core/config/aquarium_upgrades.json`，并由独立配置仓储管理
  - 鱼塘原先写死在 `main.py` 的升级路径迁入 `core/config/fish_pond_upgrades.json`
  - 新增迁移 050 和 `fish_pond_upgrades` 表；原有鱼塘容量与费用保持不变
  - 新注册用户从当前鱼塘、水族箱等级 1 读取初始容量，不再依赖代码常量

- **🖥️ Web 配置按系统归位**：
  - 新增“其他设置”页，统一管理鱼塘/水族箱档位、基础玩法、装备回收价、借贷和博弈规则
  - 银行规则放入“银行管理”，交易所容量/盈利税/波动率/重置价放入“交易所管理”
  - 玩家市场上架税率放入“市场管理”；已有“税收管理”继续独立维护税收参数
  - 运行时配置保存后立即应用，并通过 `AstrBotConfig.save_config()` 持久化；文件回退采用精确路径和原子替换
  - 配置写入改为逐层深度合并，不会因为某一页保存而抹掉其它页面或未展示的配置键

- **🛡️ 容量与表单安全**：
  - 鱼塘和水族箱均可修改容量、金币、钻石、说明，追加/删除最高等级或恢复版本默认值
  - 等级必须从 1 连续递增，容量逐级增加，费用非负，等级 1 费用固定为 0
  - 档位与已有玩家容量在单个 SQLite 事务中更新；已有玩家保持原等级并同步新容量
  - 缩容后装不下现有鱼、或删除仍有玩家使用的等级时拒绝整次保存并回滚
  - 所有新增设置路由要求管理员会话并校验 CSRF 令牌

- **⚙️ 配置一致性修复**：
  - 装备回收价 Web 配置补齐 6–10 星 schema，鱼竿与饰品下一次出售立即使用新值
  - 修复交易所波动率读取旧的 `commodities.*.volatility` 路径，改为读取现有 `exchange.volatility.*`
  - 每日重置小时变更会同步刷新后台任务观察点；借贷、骰宝、21 点、拉杆机的构造期缓存同步更新

- **🧪 验证**：
  - 全量测试通过：154 项测试、7 组子测试
  - 在运行库的只读备份上验证 schema 49 → 50：5 个鱼塘档位正确，`integrity_check=ok`、`foreign_key_check=0`
  - 验证期间未修改运行中的 schema 49 数据库

## v2.6.6 (SQLite 并发与低频维护加固)

本版是不改变现有玩法与部署架构的稳定性补丁，重点降低 SQLite 写锁竞争、缩短钓鱼热事务，
并补齐后台线程与插件卸载时的连接回收。继续使用 SQLite，不引入 PostgreSQL、Redis，
也不通过 `asyncio.to_thread` 扩大数据库并发。

- **🔁 仓储事务统一**：
  - 用户、日志、借贷、红包、抽卡、成就、增益、交易所、物品模板等仓储统一由 `DatabaseConnectionManager` 管理连接
  - 库存、商店与市场仓储的剩余写路径同步纳入统一事务，写锁冲突时可在 30 秒总预算内重放完整事务
  - 读取统一复用线程本地连接；事务异常整体回滚，不再遗留半完成写入或占用写锁

- **🧹 钓鱼记录冷热分离**：
  - 从每次钓鱼结算事务中移除全库 30 天历史清理，热路径只保留当前用户最近 50 条记录
  - 历史记录改由每小时低频维护任务分批清理，每批最多 1000 条，避免一次长事务阻塞玩家操作

- **📇 数据库索引迁移**：
  - 新增迁移 049，为 `fishing_records(timestamp)` 建立历史清理索引
  - 为开启自动钓鱼的用户建立部分索引 `idx_users_auto_fishing`，降低轮询扫描成本

- **🧵 连接生命周期收口**：
  - 成就、交易所价格、自动钓鱼与每日税收线程退出时，在各自线程中关闭线程本地连接
  - 插件终止时关闭全部仓储连接，避免重载后残留文件句柄和事务状态

- **🐛 定时红包清理修复**：
  - 修正清理任务使用旧字段名导致的每小时 SQL 报错，按当前 schema 正确删除过期 24 小时以上的红包及领取记录

- **🧪 验证**：
  - 全量测试通过：133 项测试、7 组子测试
  - 使用现有数据库一致性副本验证 47 → 48 → 49 升级路径，`integrity_check` 与 `foreign_key_check` 均通过

## v2.6.5 (银行经济闭环 + 记账与准入加固)

合并 dev 前对 v2.6.4 新增功能的一轮完善，重点是补齐生命周期缺口、把资金流水变得可审计，
并修一批与银行无关的稳定性问题。

本版完善的对象是 [@LoCCai](https://github.com/LoCCai) 在
[#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17) 中贡献的银行系统、
资产税范围配置与 SQLite 事务加固，银行、税收、借贷的主体实现均出自该 PR，再次感谢。

**关于默认玩法口径**：银行的定位是「资产税缓冲 + 金币蓄水池」，玩家把金币存入银行可以规避
每日资产税——这是 #17 的既定设计，`tax.asset_scope` / `deduct_scope` 默认值均恢复为 `wallet`，
与贡献者的原始设定一致。不希望玩家靠银行避税的服务器，把 `asset_scope` 改成 `wallet_bank`
或 `wallet_bank_fixed` 即可。同样地，大额取款预约门槛按**单笔**判定也是既定设计，本版予以保留。

- **💸 借贷催收可穿透银行**：
  - 强制收款改为按 钱包 → 银行活期可用余额 → 强制解约定期（`loan.collect_from_fixed`，默认开）的顺序扣款
  - #17 的说明中未涉及借贷交互，此前催收只看钱包，借款人把钱存进银行即可让强制收款颗粒无收，属于系统间的意外交互而非既定玩法。需要保留「银行资金不可被催收」的服务器可关闭该开关

- **🧾 欠税从此有始有终**：
  - 每日税收改为「先还旧账后缴新税」：先计滞纳金，再用可扣资产冲抵历史欠税，剩余部分缴当日税。此前只挂账不清偿，欠税会永远滚下去
  - 新增 `tax.debt_surcharge_rate`（默认 2%/天）欠税滞纳金，同一结算日只累计一次；填 0 可关闭
  - 新增 `/钓鱼还税 [金额]` 主动还税命令。此前欠税只在从银行出金时才补扣，钱全在钱包里的玩家想还也还不了
  - 欠税未清时禁止存款、开定期与转账（`bank.block_inflow_when_in_debt`，默认开）
  - 后台「税收管理」页支持按用户减免或清零欠税

- **🔓 预约不再锁死资金**：
  - 预约新增过期时间（`bank.reservation_expire_hours`，默认可确认时间后 72 小时），超时自动作废并释放锁定额。此前建表里预留的 `expired` 状态从未被写入，一笔永不确认的预约会把资金永久锁死：取不出、开不了定期、也扣不到税，只会不断累积欠税
  - 每日结算与读取预约时都会惰性回收过期预约
  - 后台「银行管理」页支持强制取消卡住的预约

- **🏦 关闭银行不再锁死玩家资产**：
  - `bank.enabled = false` 现在只封锁入金（存款、开定期），取款、定期领取与提前取出保持可用。此前一并关掉，玩家已存入的钱会被困在一个不可用的系统里，同时还在继续被扣税

- **📒 银行流水与记账补齐**：
  - 新增 `bank_transactions` 流水表，记录存取、预约锁定/释放、定期开立/结算/解约、利息、手续费、违约金、欠税挂账/补扣/减免与管理员调整，含每笔的期末余额快照
  - 定期提前取出的违约金此前只扣钱不记账，现在写入税收记录（`定期违约金`）
  - 定期利息作为「新增货币」单独留痕，便于核对通胀
  - 后台「银行管理」页新增流水表与管理员调整活期余额入口

- **⚙️ 稳定性与一致性**：
  - 银行仓储改用统一的 `DatabaseConnectionManager`：单事务 + 锁冲突自动重放，不再每次调用新建连接、也不再每次连接都切换 `journal_mode`（该做法此前已因抢锁问题从连接管理器中移除）。此前 `with sqlite3.connect(...)` 只提交事务不关闭连接，连接会一直泄漏
  - 银行时间基准由 naive `datetime.now()` 统一为 `get_now()`（UTC+8），与插件其它模块对齐；历史 naive 数据按服务器本地时区解释后换算，避免 UTC 部署下时间显示差 8 小时
  - `asset_scope` / `deduct_scope` / `taxable_mode` 增加白名单校验，非法值告警并回落默认，不再静默改变全服计税结果
  - 插件终止时一并释放银行仓储连接

- **🖥️ Web 后台修复**：
  - 税收配置写回改为优先走框架的 `save_config()`；退化写文件时按 `plugin_id` **精确定位**自己的配置文件，不再用 `astrbot_plugin_fishing*` 通配匹配——通配会把同机器上其它实例（如 `astrbot_plugin_fishing_again`）的配置一起覆盖
  - 配置写入由整段替换改为合并，不再抹掉表单不认识的键
  - 税收与银行的管理表单加上 CSRF 令牌校验
  - 关闭税收任务时的线程 join 改为 `asyncio.to_thread`，不再阻塞整个后台事件循环最多 5 秒

- **🙌 玩家侧体验**：
  - `/钓鱼银行` 概览新增可用余额、预约锁定额与欠税，并提示还税方式。此前欠税补扣是静默发生的，玩家只会看到「取款到账少了一截」
  - 存取款支持「全部」；缺少金额时的报错提示改为可读的用法提示
  - 新增 `/钓鱼银行 流水` 查看最近资金明细
  - 新增 `/排行榜 总资产`：金币榜只看钱包，银行上线后越有钱的人越会把钱藏进银行，榜单会失真

- **🧪 测试**：
  - 新增银行经济规则、服务层准入规则、借贷催收穿透银行三组回归测试（共 60+ 用例）
  - 测试用的表结构改为直接跑真实迁移的 `up()`，避免 schema 与迁移脱节
  - 修复测试在 Windows 上因连接未关闭导致临时目录删除失败、断言通过却整片报错的问题
  - 以 12 项变异测试复核用例有效性，全部被杀掉
...
- **🏦 新增银行系统**：
  - 银行活期账户，支持钱包与银行之间存取款，存款免费且即时到账
  - 每日免费提现额度（默认 1,000,000 金币），超出部分默认收取 3% 取款手续费，手续费计入税收记录
  - 单笔 5,000,000 金币及以上取款需先预约，默认 24 小时后可确认取出，预约期间对应资金锁定
  - 定期存款支持 1/3/7/30 天档位（默认收益 0.1% / 0.4% / 1.0% / 5.0%），到期领取本金与收益
  - 定期提前取出不获得收益，本金超过 1,000,000 金币的存单默认扣除 1% 违约金
  - Web 后台新增「银行管理」页，展示活期总额、开户数、进行中定期本金、预计待付收益与存单明细；用户管理页同步展示银行资产
  - 新增命令：`/钓鱼银行`（存款、取款、预约取款、确认预约、取消预约、定期、定期存款、定期列表、定期取出、提前取出）、`/钓鱼存款`、`/钓鱼取款`
  - 感谢 [@LoCCai](https://github.com/LoCCai) 的贡献 ([#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17))

- **🧾 每日资产税范围与扣款来源可配置**：
  - 新增 `asset_scope` 控制计税范围：`wallet` / `wallet_bank`（默认）/ `wallet_bank_fixed`
  - 新增 `deduct_scope` 控制扣款来源：`wallet` / `bank` / `wallet_bank`（默认）
  - 新增 `taxable_mode` 控制计税模式：`total` 按全部资产计税，`excess` 仅对超出起征点的部分计税
  - 可扣来源不足时记为欠税，后续取款、确认预约、定期领取或提前取出时优先补扣
  - Web 后台新增「税收管理」页，可直接调整税率曲线并预览各档位税额
  - 银行活期默认参与每日资产税；如需让银行余额免税，将 `asset_scope` 改回 `wallet`
    （注：该默认值已在 v2.6.5 恢复为 `wallet`，与 [#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17) 的原始设定一致）
  - 感谢 [@LoCCai](https://github.com/LoCCai) 的贡献 ([#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17))

- **🐛 银行计费时点修复**：
  - 预约取款的手续费改为确认时按当日剩余免费额度重算。此前手续费在下单时即冻结，跨过每日重置后同一份免费提现额度会被预约和普通取款各使用一次，也可能在额度已用尽时多收玩家费用
  - 取款手续费与定期违约金改为在数据库事务内计算，不再由服务层在事务外算好后传入

- **🔒 SQLite 事务与锁加固**：
  - 钓鱼结算、卖鱼、装备变现和智能扣除改为原子操作，任一环节失败整体回滚
  - 借贷事务失败后回滚并释放写锁，创建借条前校验借贷双方是否已注册
  - 数据库连接的异常路径补上 rollback，避免遗留写锁；连接建立时不再反复切换 `journal_mode`
  - 自动钓鱼按用户隔离，单个用户失败不再中断其他用户
  - 新增回滚与锁释放的回归测试
  - 感谢 [@LoCCai](https://github.com/LoCCai) 的贡献 ([#17](https://github.com/Akiyo-dayo/astrbot_plugin_fishing/pull/17))
